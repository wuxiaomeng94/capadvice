...
import com.capinfo.utils.FileInputParams;
...
public class CapAdviceController extends BaseController {
    ...
    @ResponseBody
    public JsonUtil saveDeal(CapAdviceDeal capAdviceDeal, HttpServletRequest request) {
        ...
        try {
            ...
            String id = capAdviceDealService.saveDeal(capAdviceDeal);
            //根据id，保存对应的附件
            FileInputUtils.updateFile(id, filesDynCode, "cap_advice_deal");
     ...
     */
    @PostMapping(value = "updateAdvice")
    public String updateAdvice(CapAdvice capAdvice, HttpServletRequest request) {
        CapAdvice oldCapAdvice = capAdviceService.selectByPrimaryKey(capAdvice.getId());
        BeanUtil.copyNotNullBean(capAdvice, oldCapAdvice);
        if ("1".equals(oldCapAdvice.getType())) {
            ...
            capAdviceService.updateByPrimaryKeySelective(oldCapAdvice);
            String filesDynCode = request.getParameter("filesDynCode");
            FileInputUtils.updateFile(capAdvice.getId(), filesDynCode, "cap_advice");

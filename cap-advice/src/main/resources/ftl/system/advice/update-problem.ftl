...
<script>
  ...
  layui.use([ 'layer', 'table'], function () {
    ...
    if (!detailFlag) {
        ...
        table.render({
            ...
            , cols: [[
                ...
                , {field: 'createByStr', title: '处理人姓名', width: '20%', sort: true}
                , {field: 'status', title: '处理情况提交状态', width: '20%', templet: '#statusTemp'}
                , {field: 'createDate', title: '处理时间', width: '20%',templet: '<div>{{ layui.laytpl.toDateString(d.createDate,"yyyy-MM-dd HH:mm:ss") }}</div>'}
        ...
        table.render({
            ...
            , cols: [[
                {field: 'content', title: '处理情况', width: '20%', sort: true}
                , {field: 'createDate', title: '处理时间', width: '20%',templet: '<div>{{ layui.laytpl.toDateString(d.createDate,"yyyy-MM-dd HH:mm:ss") }}</div>'}
                , {field: 'remark', title: '操作', width: '60%', toolbar: "#toolBarDetail"}
            ]]
            , page: false
            ,  height: 'full-83'
        });
    ...
    $("#sub").click(function () {
        ...
        var id = $("#id").val();
        //先是一个confirm
        layer.confirm('结案后案件办结，不能再追加处理信息，请确认是否结案？', {
            btn: ['是','否']
        }, function () {
            $.ajax({

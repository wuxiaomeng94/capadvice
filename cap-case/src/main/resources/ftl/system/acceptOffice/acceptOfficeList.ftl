...
<div class="lenos-search">
    ...
    <div class="select" id="selectForm">
        ...
        <div class="layui-inline">
            <input class="layui-input" name="name" height="20px" id="name" autocomplete="off">
        </div>
...
<script>
    ...
    layui.use(['table','laydate'], function () {
        ...
        table.render({
            id: 'adviceList',
            elem: '#adviceList'
            , url: '/acceptOffice/showOfficeList'
            , cols: [[
                //{checkbox: true, fixed: true, width: '5%'}
                 {field: 'name', title: '简称', width: '20%', sort: true}
                , {field: 'fullName', title: '全称', width: '40%', sort: true}
                , {field: 'createDate', title: '创建时间', width: '20%',templet: '<div>{{ layui.laytpl.toDateString(d.createDate,"yyyy-MM-dd HH:mm:ss") }}</div>',sort: true}
        ...
        table.on('tool(user)', function (obj) {
            var data = obj.data;
            if (obj.event === 'detail') {
                detail('编辑', '/acceptOffice/updateOffice?id=' + data.id, 700, 450);
            } else if (obj.event === 'del') {
                layer.confirm('确定删除?', function(){
                    del(data.id);
                });
            } else if (obj.event === 'edit') {
                update('编辑', '/acceptOffice/updateOffice?id=' + data.id, 700, 450);
            }
        });
    ...
    function addCase() {
        layer.open({
            ...
            shade: 0.4,
            title: '获取数据',
            content: '/acceptOffice/addOfficePage',
            end: function () {
                location.reload();
            }
        });

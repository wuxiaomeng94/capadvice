...
<div class="lenos-search">
    ...
    <div class="select" id="selectForm">
        ...
        派单时间：
        <div class="layui-inline">
            <input type="text" name="yesswCreatetimeStart" id="yesswCreatetimeStart" lay-verify="date" autocomplete="off" class="layui-input">
        </div>
        至：
        <div class="layui-inline">
            <input type="text" name="yesswCreatetimeEnd" id="yesswCreatetimeEnd" lay-verify="date" autocomplete="off" class="layui-input">
        </div>
        案件状态：
        <div class="layui-inline">
        ...
        <button class="select-on layui-btn layui-btn-sm red-btn" id="loadYesswData" onclick="loadYesswData()">
            获取数据
        </button>
        <button class="select-on layui-btn layui-btn-sm red-btn" onclick="expExcel()">
            导出excel
        </button>
        <#--<button class="select-on layui-btn layui-btn-sm red-btn">
            导入满意度excel
        </button>-->
...
<script>
    ...
    layui.use(['table','laydate'], function () {
        ...
        table.render({
            id: 'adviceList',
            elem: '#adviceList'
            , url: '/case/showYesswList'
            , cols: [[
                //{checkbox: true, fixed: true, width: '5%'}
                 {field: 'yesswNumber', title: '工单编号', width: '15%', sort: true}
                , {field: 'yesswTitle', title: '案件标题', width: '20%', sort: true}
                , {field: 'yesswCreateTime', title: '案件派单时间', width: '15%', sort: true}
                , {field: 'yesswAskedStreet', title: '被反映街道', width: '10%', sort: true}
                , {field: 'acceptOffice', title: '承办单位', width: '10%', sort: true}
                , {field: 'yesswStatus', title: '案件状态', width: '15%', sort: true}
                //, {field: 'createDate', title: '提交时间', width: '10%',templet: '<div>{{ layui.laytpl.toDateString(d.createDate,"yyyy-MM-dd") }}</div>',sort: true}
        ...
        //日期
        laydate.render({
            elem: '#yesswCreatetimeStart',
            type: 'datetime'
        });
        laydate.render({
            elem: '#yesswCreatetimeEnd',
            type: 'datetime'
        });
        ...
        var $ = layui.$, active = {
            ...
            update: function () {
                ...
                update('编辑', 'updateAdvice?id=' + data[0].id, 700, 450);
            },
            detail: function () {
                var checkStatus = table.checkStatus('roleList')
                        , data = checkStatus.data;
                ...
                detail('查看信息', '/case/editPage?id=' + data[0].id, 700, 450);
            }
        };
        ...
        table.on('tool(user)', function (obj) {
            var data = obj.data;
            if (obj.event === 'detail') {
                detail('查看', '/case/editPage?id=' + data.id, 700, 450);
            } else if (obj.event === 'del') {
                layer.confirm('确定删除?', function(){
                    del(data.id);
                });
            } else if (obj.event === 'edit') {
                update('编辑', '/case/editPage?id=' + data.id, 700, 450);
            }
        });
    ...
    function expExcel() {
        ...
        let yesswCreatetimeEnd = $("#yesswCreatetimeEnd").val();
        let yesswStatus = $("#yesswStatus").val();
        location.href="/case/expExcel?yesswNumber="+yesswNumber+"&yesswCreatetimeStart="+yesswCreatetimeStart+"&yesswCreatetimeEnd="+yesswCreatetimeEnd+"&yesswStatus="+yesswStatus;

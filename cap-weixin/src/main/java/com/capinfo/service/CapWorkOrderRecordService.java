...
public class CapWorkOrderRecordService extends BaseServiceImpl<CapWorkOrderRecord, String> {
    ...
    private void saveSpendtime(String id, String taskName, VehicleFlowEntity flow) {
        ...
        //算一下耗时
        //countDuration(spend);
        //这里的耗时，应该是暂时需要找到上一条speingtime数据的时间点，计算两个时间查得出。
        //因为签收操作要和完成检测操作在一步，需要这样来算
        countDurationByTime(spend);
    ...
    public void countDurationByTime(CapVehicleSpendtime spend) {
        CapVehicleSpendtime spendtimeQuery = new CapVehicleSpendtime();
        spendtimeQuery.setCapWorkRecordId(spend.getCapWorkRecordId());
        spendtimeQuery.setSortType("createDateAsc");
        List<CapVehicleSpendtime> spendtimeList = capVehicleSpendtimeService.selectListByCondition(spendtimeQuery);
        int index = 0;
        for (int i = 0; i < spendtimeList.size(); i++) {
            CapVehicleSpendtime spendtime = spendtimeList.get(i);
            if (spendtime.getId().equals(spend.getId())) {
                index = i;
                break;
            }
        }
        if (index>0) {
            //不是在入场车检那一步的情况下进这个方法。计算耗时就找上一条数据的开始时间到这一条数据的开始时间
            CapVehicleSpendtime lastData = spendtimeList.get(index-1);
            Date lastStartTime = lastData.getStartTime();
            Date startTime = spend.getStartTime();
            int second = (int)(startTime.getTime() - lastStartTime.getTime()) / 1000;
            spend.setDuration(second);
        }

...
public class CapVehicleController {
    ...
    public void expExcel(CapVehicleInfo capVehicleInfo, HttpServletRequest request, HttpServletResponse response) {

        request.getSession().removeAttribute("endflag");
        request.getSession().setAttribute("endflag", "0");

        List<CapVehicleInfo> capVehicleInfoList = capVehicleInfoService.selectListByCondition(capVehicleInfo);
        List<ExcelEntity> excelEntityList = new ArrayList<>();
        for (int i = 0; i < capVehicleInfoList.size(); i++) {
            CapVehicleInfo vehicleInfo = capVehicleInfoList.get(i);
            ExcelEntity excelEntity = capVehicleInfoService.loadExcelEntity(vehicleInfo);
            excelEntity.setIndex((i+1)+"");
            excelEntityList.add(excelEntity);
        }
        ExportParams params = new ExportParams("车辆检测各环节用时综合报表", "Sheet1", ExcelType.XSSF);
        try {
            Workbook workbook = ExcelExportUtil.exportBigExcel(params, ExcelEntity.class, excelEntityList);
            //Workbook workbook = capYesswCaseInfoService.bigExcel(1, capYesswCaseInfo, null, params);
            ExcelExportUtil.closeExportBigExcel();
            //下载方法
            ExcelUtils.export(response, workbook, "检测场报表");
